...
- **Design pass** (`6cd2693`): Meridian identity via ui-kit theme tokens (brass-on-ink dark palette, Space Grotesk headings, Plex Mono data); CHS ui-kit components vendored as live code; ConfirmDialog → kit AlertDialog; all surfaces tokenized with 44px targets, pending states, kit empty/skeleton states. CONTEXT.md glossary created; stub routes retired; RSS autodiscovery on lens pages.
