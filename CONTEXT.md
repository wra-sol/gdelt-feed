...
| **Coverage** | "What is the current article set for this watch?" The one seam answering it: D1 cache → TTL policy → live GDELT fetch → throttle/outage degradation to stale cache → provenance. Callers never touch GDELT directly. Split per architecture-review #3: `coverageRead.ts` is the cache-only surface (RSS/trends — physically cannot fetch); `coverage.ts` holds the live half (swr/revalidate/warm). | `services/coverageRead.ts`, `services/coverage.ts` |
| **Stale-cache** | Degraded coverage source: upstream throttled/failed, serving the last known payload with `stale: true`. Honesty rule: one missed warm window (cache age <2×15 min) is **not** degraded — it sits inside GDELT's own index cycle; only ≥2 consecutive missed windows earn the banner. Surfaces must show the throttle state honestly. | `coverage.ts`, badges on lens cards |
...
| **Timeline** | Per-watch volume history ("how has volume moved?"): DOC timelinevol cached under the same window policy as Coverage (`timeline_cache`, cron-warmed). The other watch-keyed upstream read; Search is the one documented direct caller instead (ADR-002). | `services/timeline.ts`, `services/timelineCache.ts` |
